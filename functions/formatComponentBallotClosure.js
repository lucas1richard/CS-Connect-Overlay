...
export default function formatComponentBallotClosure() {
  ...
  var results = [];
  // Display results
  Object.keys(votingrecords.votingCommittees).forEach(key => {
    var txt = [key];
  ...
  function getStatArr(num, nextR, currentR) {
    ...
    return [
      nextRow.children[num].innerText,
      currentRow.children[num].innerText
    ];
  }
  ...
  function getSummaryArray() {
    var summaryArray = [];
    Object.keys(votingrecords.records).forEach(key => {
      var voteTxt = [];
      var commentTxt = [];
      var outTxt = `Record ${key} was '`;

      Object.keys(votingrecords.records[key]).forEach(com => {
        if (typeof votingrecords.records[key][com] === 'string') {
          commentTxt.push(`${votingrecords.records[key][com]} from ${com}`);
        } else {
          let stat = votingrecords.records[key][com] ? `approved by the ${com}` : `disapproved by the  ${com}`;
          voteTxt.push(stat);
        }
      });
  ...
  function saveBallotClosure() {
    ...
    chrome.storage.local.get({
      ...
      res.recentBallots = res.recentBallots.filter(ballot => ballot.ballotNum !== ballotNumber);
